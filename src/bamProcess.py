...
handler.setFormatter(logging.Formatter(
	'[ {asctime}] {levelname:8s} {filename} {message}\n',
	style='{'))
...
def jointCall(para):
	...
	cmd1 = bcftools + " mpileup -b " + bam_filter + " -f "  + reference + " -r " +  jobid + " -q 20 -Q 20 -d 10000 -Ou"
...
def BamFilter(para):
	...
	max_mismatch = para['max_mismatch']
	samtools = para['samtools']
	
	# Create output directory if it doesn't exist
	os.makedirs(out + "/Bam", exist_ok=True)
	
	# Output filtered BAM file path
	output_bam = out + "/Bam/" + sample_id + "_" + chr + ".filter.bam"
	
	# Use samtools to filter BAM file for specific chromosome
	# Filter for mapping quality >= 20, base quality >= 20, and specific chromosome
	cmd = f"{samtools} view -b -q 20 -h {bam_file} {chr} > {output_bam}"
	
	logger.debug(f"Filtering BAM for sample {sample_id}, chromosome {chr}")
	...
	if result == 0:
		...
		if index_result == 0:
			logger.debug(f"Successfully filtered and indexed BAM for {sample_id}_{chr}")
			return output_bam
		else:
			logger.error(f"Failed to index filtered BAM for {sample_id}_{chr}")
			return None
	else:
		logger.error(f"Failed to filter BAM for {sample_id}_{chr}")
		return None
...
def preProcess(args):
	...
	# Define samtools path
	samtools = args.apps + "/samtools"
	...
	with open(args.bamFile) as f_in:
		for line in f_in:
			...
			for chr in range(1, 23):
				para_single = dict(chr = "chr" + str(chr), out = args.out, id = record[0], bamFile = record[1], max_mismatch = args.max_mismatch, samtools = samtools)
